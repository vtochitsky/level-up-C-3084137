...
#include <time.h>
#include "functions.h"
/* using MUTEX to protect corruption of common global buffer */
...
pthread_mutex_t shared_buf_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void *call_left(void *args)
{
  ...
  for (size_t i = 0u; i < N_LEFT; i++)
  {
    ...
    pthread_mutex_lock(&shared_buf_mutex);
    strcpy(buf, left(targs->input, targs->len));
    pthread_mutex_unlock(&shared_buf_mutex);
